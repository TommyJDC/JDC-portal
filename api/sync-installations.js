...
const TASK_NAME = 'sync-installations';
...
export const handler = async (event, context) => {
  console.log(`[${TASK_NAME}] Début de la synchronisation`);
  ...
  try {
    ...
    if (!taskState || (taskState.lastRun && taskState.lastRun.toDate() < oneHourAgo)) { // Vérifier l'existence de lastRun
      ...
      console.log(`[${TASK_NAME}] Synchronisation terminée avec succès`);
      return result;
    ...
    console.error(`[${TASK_NAME}] Erreur de synchronisation:`, error);
    return {
      statusCode: 500,
